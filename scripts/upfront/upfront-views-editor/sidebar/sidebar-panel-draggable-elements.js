    ...
    return SidebarPanel.extend({
        ...
        get_elements: function () {
            ...
            if ( this.sections ){
                this.sections.each(function(section){
                    if ( section.elements.length ) // .size() durch .length ersetzt
        ...
        update_sections: function () {
            ...
            var me = this,
                section_have_els = 0;
            this.sections.each(function(section){
                if ( section.elements && section.elements.length > 0 ){ // .size() durch .length ersetzt
                    section_have_els++;
                }
                else {
        ...
        elements_are_initialized: function(elements) {
            return elements.length > 0; // .size() durch .length ersetzt
        },
        _reset_modules: function () {
            ...
            if ( region.get("modules").length === elements.length) return; // .size() durch .length ersetzt
            var self = this;
