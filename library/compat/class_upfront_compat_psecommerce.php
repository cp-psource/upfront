...
class Upfront_Compat_PSeCommerce {
	...
	public function add_hooks() {
		if (class_exists('PSeCommerce') === false) {
	...
	/**
	 * Hides PSeCommerce layouts in builder exported layouts if "Layouts" popup when PSeCommerce is not active.
	 */
	...
	/**
	 * PSeCommerce product post type getter
	 *
	...
	public function get_sample_content($specificity) {
		ob_start();
		include(get_theme_root() . DIRECTORY_SEPARATOR . 'upfront'. DIRECTORY_SEPARATOR . 'library' . DIRECTORY_SEPARATOR . 'compat' . DIRECTORY_SEPARATOR . 'psecommerce' . DIRECTORY_SEPARATOR . $specificity . '.php');
		return  ob_get_clean();
	}
	...
	/**
	 * Overrides the entity IDs when we're dealing with PSeCommerce output
	 *
	...
	public function override_entity_ids ($cascade) {
		// Let's test if a theme supports PSeCommerce product layouts.
	...
	public function layout_to_name($layout_name, $type = '', $item = '', $specificity = '') {
		...
		if ($item === 'mpproduct') {
			return __('PSeCommerce Product', 'upfront');
		}

		if ($item === 'mpproduct_tag') {
			return __('PSeCommerce Tag Archive', 'upfront');
		}

		if ($item === 'mpproduct_category') {
			return __('PSeCommerce Category Archive', 'upfront');
		}

		if ($specificity === 'single-page-mpproducts' || $specificity === 'mpproducts') {
			return __('PSeCommerce Products Page', 'upfront');
		}
		...
		if ($specificity === 'single-page-mpstore' || $specificity === 'mpstore') {
			return __('PSeCommerce Store Page', 'upfront');
		}

		if ($specificity === 'single-page-mpcheckout' || $specificity === 'mpcheckout') {
			return __('PSeCommerce Checkout Page', 'upfront');
		}

		if ($specificity === 'single-page-mporderstatus' || $specificity === 'mporderstatus') {
			return __('PSeCommerce Order Status Page', 'upfront');
		}
	...
	function add_layouts($layouts) {
		...
		$layouts['psecommerce'] = array(
			'pluginName' => 'PSeCommerce',
